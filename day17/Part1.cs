...
namespace day17
{
    public class Part1
    {
        ...
        public static int Dijkstra(List<List<int>> map)
        {
            ...
            while (movements.Count > 0)
            {
                ...
                if (!visited.ContainsKey((L.R, L.C, D, S)))
                {
                    ...
                    foreach (var direction in directions)
                    {
                        ...
                        if (directions.TryGetValue(D, out (int R, int C) d) && (d.R + direction.Value.R, d.C + direction.Value.C) == (0, 0)) continue; // prevent moving backwards
            ...
            return visited.Where(e => (e.Key.R, e.Key.C) == destination).First().Value;
        }
    }
}