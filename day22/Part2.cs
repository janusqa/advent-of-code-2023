...
namespace day22
{
    public class Part2
    {
        public static int Result()
        {
            ...
            foreach (var disentegrated in bricks)
            {
                var remaining = bricks.ToList();
                remaining.Remove(disentegrated);
                foreach (var (brick, idx) in remaining.Select((b, idx) => (b, idx)).ToList())
                {
                    if (OnGround(brick)) continue;
                    int newPosition = Settle(brick, remaining);

                    if (brick.s2.z >= brick.s1.z)
                    {
                        remaining[idx] = ((brick.s1.x, brick.s1.y, newPosition), (brick.s2.x, brick.s2.y, newPosition + (brick.s2.z - brick.s1.z)));
                    }
                    else
                    {
                        remaining[idx] = ((brick.s1.x, brick.s1.y, newPosition + (brick.s2.z - brick.s1.z)), (brick.s2.x, brick.s2.y, newPosition));
                    }
                    if (remaining[idx] != brick) result++;
                }
            }
    ...
    )
        {
            ...
            return below.Any() ? below.Max() + 1 : 1;
        }
