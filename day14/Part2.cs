    ...
    public class Part2
    {
        public static int Result()
        {
            ...
            for (int i = 0; i < numCycles; i++)
            {
                ...
                {
                    Tilt(platform, directions, direction.Key);
                }

                string platformHash = string.Join("|", platform.Select(r => $"{string.Join("", r)}")); // we need to track this hash of the grid (or grid itself) (memoize)
                int startOfCycle = memo.IndexOf(platformHash); // sif we see a load repeated then the first time it occured is here
                if (startOfCycle != -1)
                {
                    // !!! cycle detection !!! 
                    // We have arrived at an instance that we have already seen in our memo 
                    // Now calculate where the 1000000000 cycle will be by using mod math.
                    // We calculate the length of the sequence.
                    // We now subtract the index into memo we want (1000000000-1) from the start of the cycle
                    // This will leave us with just the cyle part of the array.
                    // we can now mod this large index with the cycle length to find where in the cycle the large index maps too.
                    // we now add back on the first part we originally took off and the result of this is the index into memo we need
                    int answerIndex = (((numCycles - 1) - startOfCycle) % (i - startOfCycle)) + startOfCycle;
                    result = Load(memo[answerIndex].Split("|").Select(r => r.ToCharArray().ToList()).ToList());
                    break;
                }
                memo.Add(platformHash);
            }
